...
static unsigned long int seed = 10000000;  /* Зерно для генерации псевдо случайных чисел */

/*============================== Функции ==============================*/
/* Генерация псевдослучайных чисел линейным конгруэнтным методом (младшие биты меняются плохо)
 * Генерирует число от 0 до m (период такой последовательности равен примерно m) */
unsigned int randLcg(void)
{
	// Возвращаем новое число по формуле: X(n+1) = a*X(n) mod m	
	seed = seed * 16807U % (MAX_RAND_NUM+1);
	return seed;
}
